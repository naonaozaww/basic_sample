...
class PasswordResetsController < ApplicationController
  ...
  def create 
    ...
    redirect_to(root_path, success: t('defaults.message.password_reset_mail_send'))
  end
  ...
  def update
    ...
    if @user.change_password(params[:user][:password])
      redirect_to login_path, success: t('defaults.message.change', item: User.human_attribute_name(:password))
    else
      render :edit
    end
  end
